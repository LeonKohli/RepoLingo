<template>
  <Transition name="modal">
    <div v-if="show" class="fixed inset-0 z-50 flex items-center justify-center overflow-auto bg-black bg-opacity-50">
      <div class="relative w-full max-w-md p-8 m-4 border border-white rounded-lg shadow-xl bg-background-dark bg-opacity-90 backdrop-filter backdrop-blur-sm border-opacity-10">
        ...
        <a href="https://github.com/settings/tokens" target="_blank" rel="noopener noreferrer" class="inline-block mb-4 text-primary hover:underline">
          Get a token here
        </a>
        <input 
          v-model="apiKey"
