<template>
  <section
    class="p-6 transition-all duration-300 bg-[#0f172a] bg-opacity-50 shadow-lg rounded-2xl hover:shadow-xl border border-white border-opacity-10 backdrop-filter backdrop-blur-sm">
    ...
    <form @submit.prevent="$emit('fetch-repo')" class="space-y-6">
      <div>
        ...
        <div class="relative">
          <input :value="repoUrl" @input="updateRepoUrl($event.target.value)" type="url" id="repo-url"
          ...
          <Icon name="uil:github"
            class="absolute text-gray-400 transition-all duration-300 ease-in-out transform -translate-y-1/2 left-3 top-1/2" />
        </div>
      </div>
      <div>
